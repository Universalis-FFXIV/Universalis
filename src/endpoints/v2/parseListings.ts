	...
	for (let i = data.items.length - 1; i >= 0; i--) {
		...
		if (isDC) {
			...
			if (otherItemOnDC) {
				...
				otherItemOnDC.recentHistory = otherItemOnDC.recentHistory.concat(
					item.recentHistory,
				);
...
function calculateAveragePrices(
	...
	const stats = calcTrimmedStats(
		calcStandardDeviation(...historicalPpu),
		...historicalPpu,
	);
	const statsNQ = calcTrimmedStats(
		calcStandardDeviation(...nqHistoricalPpu),
		...nqHistoricalPpu,
	);
	const statsHQ = calcTrimmedStats(
		calcStandardDeviation(...hqHistoricalPpu),
		...hqHistoricalPpu,
	);
