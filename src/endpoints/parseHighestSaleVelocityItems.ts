...
import { capitalise, calcSaleVelocity } from "../util";
import { WorldItemPair } from "../models/WorldItemPair";
import validation from "../validate";
import { Cursor, Collection } from "mongodb";

export async function parseHighestSaleVelocityItems(ctx: ParameterizedContext, worldMap: Map<string, number>, worldIDMap: Map<number, string>, collection: Collection) {
...
async function getHighestSaleVelocity(worldID: number, count: number, worldIDMap: Map<number, string>, collection: Collection): Promise<WorldItemPair[]> {
    if (count <= 0) return [];
    ...
    const highest: Cursor = collection.find(query, { projection: { itemID: 1, worldID: 1, recentHistory: 1, lastUploadTime: 1 } }).map((item) => {
    ...
    }).sort("saleVelocity", 1).limit(Math.min(count, 20));
